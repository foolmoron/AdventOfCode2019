...
function printMap(map) {
    ...
    let minX = map.map(xs => Object.keys(xs).map(x=>parseInt(x)).reduce((acc, x) => Math.min(acc, x), Infinity))
        ...
        ;
    for (let y = map.length - 1; y >= minY; y--) {
        let str = '';
        for (let x = minX; x < map[y].length; x++) {
